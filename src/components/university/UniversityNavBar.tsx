...
/**
 * Authenticated navbar for the university portal. Mirrors ApplicantNavBar's
 * visual system — white bg, 0.91 px ghost bottom border, navy text, framer
 * layoutId underline on the active link — so the two authenticated portals
 * feel like the same app.
 *
 * Links: Dashboard · Applications. Ranking is intentionally NOT a top-level
 * link — it's always scoped to a program (/university/programs/:id/ranking),
 * so it's reached from the program context, not from the navbar. No bell
 * either yet: a dedicated /university/notifications page hasn't been built,
 * so the Dashboard surfaces recent activity inline until that page ships.
 */
...
const NAV_LINKS: NavLinkDef[] = [
  ...
  { to: '/university/programs', label: 'Applications' },
];
      ...
      <div className="mx-auto flex w-full max-w-[1366px] items-center px-6 pt-[22px] pb-[18px] md:px-[58px]">
        ...
        <div className="ml-auto flex items-center gap-[14px] md:hidden">
          ...
          <button
            ...
            aria-controls="university-mobile-drawer"
...
function MobileDrawer({ open, onClose }: MobileDrawerProps) {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div
          key="university-drawer"
          id="university-mobile-drawer"
          ref={drawerRef}
          role="dialog"
          aria-modal="true"
          aria-label="University navigation"
